...
\t'demo2'\t'demo3'\r\n You can add the word 'auto' to demo1 and demo2\r\n\r\n",
...
static portBASE_TYPE startCommand( int8_t *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString )
{
	int8_t *pcParameterString1, *pcParameterString2; uint8_t temp = 0;
	portBASE_TYPE xParameterStringLength1, xParameterStringLength2;
	
	static const int8_t *pcDemo = ( int8_t * ) "Demo %d is executing\n\r";
	static const int8_t *pcDemoMan = ( int8_t * ) "Demo %d is executing in manual mode\n\r";
	static const int8_t *pcDemoAuto = ( int8_t * ) "Demo %d is executing in automatic mode\n\r";
	...
	{
		if (!strcmp(( char * ) pcParameterString2, "auto") ) 
		{
			//demoMode = 1;
			sprintf( ( char * ) pcWriteBuffer, ( char * ) pcDemoAuto, 1);
			
			/* 1 : Set sensor mode to off */
			sensor = 0;
			
			/* 2 : Set ledmode threshold high */
			int_thres = 1000;
			
			/* 3 : Turn LED 19 on */
			LEDCode = LED_19;
			
			/* 4 : Command gimbal for random profile 1 */
			temp = 12; writePx(P5, (const char *) &temp, 1, HAL_MAX_DELAY); osDelay(10);	/* Random distribution from straight position */
			temp = 25; writePx(P5, (const char *) &temp, 1, HAL_MAX_DELAY); osDelay(10);	/* scale */
			temp = 3; writePx(P5, (const char *) &temp, 1, HAL_MAX_DELAY);		/* rate */
		...
		{
			demoMode = 0;
			sprintf( ( char * ) pcWriteBuffer, ( char * ) pcDemoMan, 1);
			...
			/* 4 : Command gimbal to turn motors off */
			temp = 15; writePx(P5, (const char *) &temp, 1, HAL_MAX_DELAY); 
	...
	else if (!strncmp(( char * ) pcParameterString1, "demo2", (size_t) xParameterStringLength1) ) 
	{
		...
		{
			//demoMode = 2;
			sprintf( ( char * ) pcWriteBuffer, ( char * ) pcDemoAuto, 2);
			
			/* 1 : Set sensor mode to on */
			sensor = 1;
			
			/* 2 : Set ledmode threshold high */
			int_thres = 1000;
			
			/* 3 : Command gimbal for random profile 1 */
			temp = 12; writePx(P5, (const char *) &temp, 1, HAL_MAX_DELAY); osDelay(10);	/* Random distribution from straight position */
			temp = 30; writePx(P5, (const char *) &temp, 1, HAL_MAX_DELAY); osDelay(10);	/* scale */
			temp = 1; writePx(P5, (const char *) &temp, 1, HAL_MAX_DELAY);		/* rate */
		...
		{
			sprintf( ( char * ) pcWriteBuffer, ( char * ) pcDemoMan, 2);
			...
			/* 3 : Command gimbal to turn motors off */
			temp = 15; writePx(P5, (const char *) &temp, 1, HAL_MAX_DELAY); 
	...
	else if (!strncmp(( char * ) pcParameterString1, "demo3", (size_t) xParameterStringLength1) ) 
	{
		...
		/* 3 : Command gimbal to go straight */
