...
uint16_t adc = 0, combined = 0, sig980 = 0, sig850 = 0; uint16_t count = 0;
uint8_t outputCombined[200] = {0}, output850[200] = {0}, output980[200] = {0};
uint8_t outputCombinedOld[200] = {0}, output850Old[200] = {0}, output980Old[200] = {0};
uint8_t sig850Width = 0, sigStart = 0;
...
{	
	...
	/* Combined signal */
	combined = adc;
	/* Receive 980nm signal from module 5 */
	Delay_us(100);
	if (dataBuffer[10] != 0xFF && dataBuffer[11] != 0xFF) 
	{
		sig980 = (dataBuffer[10]<<8) + dataBuffer[11];
	}
	
	/* Compute output overlapping signal */
	if (combined > 100)			// 100 for 980, 110 for 850, 250 for both
		outputCombined[count] = 1;
	else
		outputCombined[count] = 0;
	
	/* Compute output 850nm signal */
	if ((combined > 110 && sig980 < 50) || (combined > 250 && sig980 > 380))
		output850[count] = 1;
	else
		output850[count] = 0;	
	
	/* Compute output 980nm signal */
	if (sig980 > 380)
		output980[count] = 1;
	else
		output980[count] = 0;	
	...
	/*-- Chip signals ~ 20-50 msec, 980nm signals ~ 100msec at 5 rpm motor speed --*/
	
	/* Collect and scan 200 samples each round */

	
	/* Output the processed signals - Execute at every sample */
	
	/* Output overlapping signal at P1 TX */
	if (outputCombinedOld[count])
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_2,GPIO_PIN_SET);
	else
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_2,GPIO_PIN_RESET);
	
	/* Output 850nm signal at P2 TX */
	if (output850Old[count] )
		HAL_GPIO_WritePin(GPIOC,GPIO_PIN_4,GPIO_PIN_SET);
	else
		HAL_GPIO_WritePin(GPIOC,GPIO_PIN_4,GPIO_PIN_RESET);	
	
	/* Output 980nm signal at P3 TX */
	if (output980Old[count])
		HAL_GPIO_WritePin(GPIOC,GPIO_PIN_6,GPIO_PIN_SET);
	else
		HAL_GPIO_WritePin(GPIOC,GPIO_PIN_6,GPIO_PIN_RESET);	

	
	/* Filter the 850 signal - Execute at every round end */
	if (count == 199)
	{
		count = 0;

		/* Filter the signal - Check if 850nm output is too short */
		for(uint8_t i=0 ; i<200 ; i++)
		{
			if(output850[i] == 1) 
			{
				++sig850Width;
				if (sig850Width == 1)	sigStart = i;			// Record signal start sample
			} 
			else 
			{
				/* Is the '1' too short? */
				if (sig850Width <= 30) {			//	< xmsec at 5 rpm motor speed
					for(uint8_t j=sigStart ; j<(sigStart+sig850Width) ; j++)				// Remove the short signal
					{
						output850[j] = 0;
					}		
				}
		...
		/* Build a signal envelope for 850 */
		sig850Width = 0; sigStart = 0;
		for(uint8_t i=0 ; i<200 ; i++)
		{
			if(output850[i] == 0) 
			{
			...
			{
				/* Is the '0' too short? */
				if (sig850Width <= 100) {			//	< xmsec at 5 rpm motor speed
					for(uint8_t j=sigStart ; j<(sigStart+sig850Width) ; j++)				// Remove the short signal
					{
						output850[j] = 1;
					}		
		...
		/* Copy processed signal to prepare for output */
		memcpy(outputCombinedOld, outputCombined, 200);
		memcpy(output850Old, output850, 200);
		memcpy(output980Old, output980, 200);
	...
	/* Wait some time */
	//Delay_us(25);
	/* Send 980nm signal to module 2 */
	writePxMutex(P5, (char*) &dataBuffer[8], 2, cmd50ms, HAL_MAX_DELAY);
...
void StreamSetup(uint8_t port, float rate)
{
	...
	portStatus[P3] = STREAMING; portStatus[P6] = STREAMING; portStatus[P6] = STREAMING;
#elif (_module == 6)
	UplinkDMA1_Setup(GetUart(P6), GetUart(P3));
	portStatus[P3] = STREAMING; portStatus[P6] = STREAMING;
#elif (_module == 7)
	...
	DownlinkDMA2_Setup(GetUart(P5), 2, 10);		// 980nm signal
#elif (_module == 3)
